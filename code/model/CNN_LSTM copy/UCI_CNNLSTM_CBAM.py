...
from CNN_LSTM import CNNLSTM  # 导入CNN_LSTM
...
def train_model(model,
    ...
    for epoch in range(num_epochs):
        ...
        for step, (b_x, b_y) in enumerate(traindataloader, 1):  # 取标签和样本
            ...
            b_x = b_x.unsqueeze(1)   # sequeeze 用于对输入数据进行压缩或者unsqueeze解压，也就是增加维度或者减小维度
...
def test(model, testdataloader, criterion):
    ...
    for step, (input, target) in enumerate(testdataloader):  # 取标签和样本
        ...
        input = input.unsqueeze(1)   # sequeeze 用于对输入数据进行压缩或者unsqueeze解压，也就是增加维度或者减小维度
