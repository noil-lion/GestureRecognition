...
channels = 3
batch_size = 64  # 因为处理的图片变大,batch_size变小一点 32->24
num_classes = 5
...
VGG16_new = multi_gpu_model(VGG16_new, 2)  # GPU个数为2
VGG16_new.save('VGG16_complt'+str(epochs)+'dataV6.h5')
# 保存训练的日志文件
pd.DataFrame(history.history).to_csv('training_'+str(epochs)+'dataV6log.csv', index=False)
