...
number = 93
data_path = r'D:/ResearchSpace/TFL/dataV4/test'
model = load_model('../model/VGG16_pre_based_on_dataV4.h5')
# 模型加载


# 这个地方设置混淆矩阵的颜色主题，这个主题看着就干净~
# 混淆矩阵绘制
...
def pridict(model, data_path):
    ...
    for K in test_path_list:
        ...
        for C in test_class_path_list:
            test_image_path_list = os.listdir(data_path+'/'+K+'/'+C)
            for i in test_image_path_list[0:number]:
                test_image = image.load_img(data_path+'/'+K+'/'+C+'/'+i, target_size=(128, 128))
...
def plot_confuse(model, data_path, y_val, labels):
    ...
    print('平均识别时间：'+str((end-start)/(93*5*3)))
...
test_y = [[1.00, 0.00, 0.00, 0.00, 0.00]]*number*3   # 初始状态
test_y.extend([[0.00, 1.00, 0.00, 0.00, 0.00]]*number*3)  # qbcy
test_y.extend([[0.00, 0.00, 1.00, 0.00, 0.00]]*number*3)  # qbxq
test_y.extend([[0.00, 0.00, 0.00, 1.00, 0.00]]*number*3)  # szcs
test_y.extend([[0.00, 0.00, 0.00, 0.00, 1.00]]*number*3)  # szqs
