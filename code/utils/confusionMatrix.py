...
class ConfusionMatrix(object):
    ...
    def update(self, matrix):
        self.matrix = matrix  # 根据预测结果和真实标签的值统计数量，在混淆矩阵相应位置+1
    ...
    def summary(self):  # 计算指标函数
        ...
        for i in range(self.num_classes):  # 精确度、召回率、特异度的计算
            ...
            A_P.append(Precision)
            A_R.append(Recall)
            table.add_row([self.labels[i], Precision, Recall, Specificity])
        print(table)
        print("the model Average precision is ", sum(A_P) / len(A_P))
        print("the model Average Recall is ", sum(A_R) / len(A_R))
        return str(acc)
    ...
    def plot(self):  # 绘制混淆矩阵
        ...
        for x in range(self.num_classes):
            for y in range(self.num_classes):
                ...
                info = (matrix[y, x] / np.sum(self.matrix[x, :]))
                plt.text(x,
                         y,
                         round(info, 3),
